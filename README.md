...
Note the **access ID** returned when the auth method was created (it starts with `p-`). You'll need this to authenticate.

```bash
# Find the access ID for an existing auth method
akeyless describe-auth-method --name "/Auth/CertificateAuth" | jq -r '.access_id'
```
